...
# fix parameter
pls_spec4 <- pls(num_comp = 4, predictor_prop = 1) %>% 
  set_mode("regression") %>% 
  set_engine("mixOmics")
...
pls_spec_tune <- pls(num_comp = tune()) %>% 
  set_mode("regression") %>% 
  set_engine("mixOmics")
...
pls_fit %>%
  ...
  ggplot(aes(x = num_comp, y = mean)) +
    ...
    geom_point(size = 2) +
    scale_y_continuous(limits = c(0.10, 0.14))
...
Imp_plsr <- tibble(var = rep(names(ETdataS[,-35:-36]),4),
  loadings = c(data.frame(FItted_plsr$fit$loadings)$X.comp1,
                            ...
                            data.frame(FItted_plsr$fit$loadings)$X.comp4),
  comp = c(rep(1,34),rep(2,34),rep(3,34),rep(4,34)))
...
Mock_ETdataS <- tibble(data.frame(mvrnorm(n = length(ETdataS$ET), 
                                         colMeans(ETdataS[,-35,-36], na.rm = FALSE, dims = 1),
                                         cov(ETdataS[,-35,-36]) )))
...
PLSR_model <- 
  # specify that the model is a PLSR
  plsda() %>%
...
Plsfit1 <- Pls000
plssgrid <- expand.grid(ncomp = 1)
Plsfit1[[i]] <- train(lai~., 
                        data=data.frame(lai=(LAI00015[[i]]), GEdata000s[[i]]),
                        method = "pls", # package wrapped
                        type= "Regression",
                        tuneGrid = plssgrid,
                        trControl = nonControl, # cross-validation
